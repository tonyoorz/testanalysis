...
# AI Chat Integration - Global variables and functions
streaming_data = {}
...
def create_deepseek_client():
    ...
    access_code = os.getenv('DEEPSEEK_ACCESS_CODE', '7FD25E1BD6124A1C8BF29030C8BFC43E')
    base_url = os.getenv('DEEPSEEK_BASE_URL', 'https://aistudio.bmwbrill.cn/function-service/open-ai/deepseek-r1/v3')
    
    if not access_code:
        raise ValueError("DEEPSEEK_ACCESS_CODE environment variable is required")
...
# 流式处理DeepSeek-R1响应
def stream_deepseek_response(user_input, conversation_history, task_id):
    """流式处理API响应，实时更新思考过程"""
    ...
    messages = [
        {
            "role": "system",
            "content": "你是一个专业的缺陷管理分析助手，请用中文回答问题。你可以帮助用户分析缺陷数据、解释图表、提供建议。请详细展示你的思考过程。"
        }
    ]
            ...
            try:
                ...
                if hasattr(chunk.choices[0].delta, 'reasoning_content') and chunk.choices[0].delta.reasoning_content:
                    ...
                    with streaming_lock:
                        ...
                        streaming_data[task_id]['progress'] = 'AI正在思考...'
...
# AI数据上下文处理函数
def process_comprehensive_data(filtered_data=None):
    """
    处理缺陷和测试数据，为AI提供综合上下文
    
    Args:
        filtered_data: 过滤后的数据（通常来自回调状态）
    
    Returns:
        tuple: (defect_df, test_df) 处理后的缺陷和测试数据
    """
    global df, test_df
    
    # 处理缺陷数据
    current_defect_data = df.copy() if not df.empty else pd.DataFrame()
    
    # 如果有过滤数据，应用过滤（假设过滤数据主要针对缺陷数据）
    if filtered_data and not current_defect_data.empty:
        try:
            # 这里可以根据具体的过滤逻辑来处理
            # 由于 defect_explore 有复杂的过滤系统，我们保持数据不变
            pass
        except Exception as e:
            print(f"应用过滤时出错: {e}")
    
    # 处理测试数据（获取最新的测试数据）
    current_test_data = test_df.copy() if not test_df.empty else pd.DataFrame()
    
    return current_defect_data, current_test_data

def generate_enhanced_data_context(defect_data, test_data, context_type='comprehensive'):
    """
    生成增强的数据上下文，包含缺陷和测试数据的综合信息
    
    Args:
        defect_data: 缺陷数据DataFrame
    ...
    Returns:
        str: 格式化的数据上下文字符串
    """
    context_parts = [f"当前数据分析报告 ({context_type}):"]
    
    # 缺陷数据分析
    if not defect_data.empty:
        context_parts.append(f"\n=== 缺陷管理数据 ===")
        context_parts.append(f"• 总缺陷数: {len(defect_data)}")
        
        # 项目分布
        if 'project' in defect_data.columns:
            project_counts = defect_data['project'].value_counts().head(5).to_dict()
            context_parts.append(f"• 主要项目分布: {project_counts}")
        
        # 严重性分布
        if 'severity_group' in defect_data.columns:
            severity_counts = defect_data['severity_group'].value_counts().to_dict()
            context_parts.append(f"• 严重性分布: {severity_counts}")
        
        # 状态分布
        if 'status_phase' in defect_data.columns:
            status_counts = defect_data['status_phase'].value_counts().head(5).to_dict()
            context_parts.append(f"• 主要状态分布: {status_counts}")
        
        # AIDA分布
        if 'aida_english' in defect_data.columns:
            aida_counts = defect_data['aida_english'].value_counts().head(5).to_dict()
            context_parts.append(f"• 主要AIDA分布: {aida_counts}")
        
        # TopIssue分析
        if 'topissue_display' in defect_data.columns:
            topissue_counts = defect_data['topissue_display'].value_counts().head(3).to_dict()
            context_parts.append(f"• TopIssue分布: {topissue_counts}")
    else:
        context_parts.append("\n=== 缺陷管理数据 ===")
        context_parts.append("• 当前无缺陷数据")
    
    # 测试数据分析
    if not test_data.empty:
        context_parts.append(f"\n=== 测试管理数据 ===")
        context_parts.append(f"• 总测试用例数: {len(test_data)}")
        
        # 项目分布
        if 'project' in test_data.columns:
            test_project_counts = test_data['project'].value_counts().head(5).to_dict()
            context_parts.append(f"• 测试项目分布: {test_project_counts}")
        
        # 运行状态分布
        if 'run_status' in test_data.columns:
            run_status_counts = test_data['run_status'].value_counts().head(5).to_dict()
            context_parts.append(f"• 运行状态分布: {run_status_counts}")
        
        # AIDA覆盖
        if 'aida_english' in test_data.columns:
            test_aida_counts = test_data['aida_english'].value_counts().head(5).to_dict()
            context_parts.append(f"• 测试AIDA覆盖: {test_aida_counts}")
        
        # FV分布
        if 'fv' in test_data.columns:
            fv_counts = test_data['fv'].value_counts().head(5).to_dict()
            context_parts.append(f"• FV分布: {fv_counts}")
    else:
        context_parts.append(f"\n=== 测试管理数据 ===")
        context_parts.append("• 当前无测试数据")
    
    # 综合分析
    context_parts.append(f"\n=== 综合分析 ===")
    if not defect_data.empty and not test_data.empty:
        # 项目对比
        defect_projects = set(defect_data.get('project', []).dropna())
        test_projects = set(test_data.get('project', []).dropna())
        common_projects = defect_projects.intersection(test_projects)
        context_parts.append(f"• 共同项目: {list(common_projects)}")
        
        # AIDA对比
        defect_aidas = set(defect_data.get('aida_english', []).dropna())
        test_aidas = set(test_data.get('aida_english', []).dropna())
        common_aidas = defect_aidas.intersection(test_aidas)
        context_parts.append(f"• 共同AIDA领域: {list(common_aidas)}")
...
@performance_monitor
def filter_dataframe(df, projects=None, start_date=None, end_date=None, aidas=None, statuses=None, pus=None, testers=None, fvs=None, ecus=None, lead_models=None, fvps=None):
    """通用数据筛选函数"""
    filtered_df = df.copy()
...
def create_sidebar_nav():
    """创建可展开/隐藏的侧边导航栏"""
    nav_items = [
        {'id': 'tab-defect-status', 'label': 'Defect Status Analysis', 'icon': 'fas fa-chart-bar'},
        ...
        {'id': 'tab-testing-efficiency', 'label': 'Testing Efficiency & Quality', 'icon': 'fas fa-chart-line'},
...
def render_content(tab):
    ...
    if tab is None or tab not in ['tab-defect-status', 'tab-project-analysis', 'tab-defect-high-runner', 'tab-defect-long-runner', 'tab-testing-team', 'tab-testing-efficiency', 'tab-test-coverage', 'tab-test-status']:
    ...
    if tab == 'tab-defect-status':
        return html.Div([
            ...
            html.Div([
                ...
                html.Div([
                    ...
                    dcc.Dropdown(
                        ...
                        value=['01-New', '02-In Pre-Analysis', '03-In Analysis', '04-In Progress', '05-In Testing', '07-In Pre-Verification', '08-In Verification'], # Set default values
                        multi=True,
                        clearable=True,
            ...
            html.Div([
                html.Div([
                    ...
                    dcc.Dropdown(
                        ...
                        value=['Matrix-1A', 'Matrix-1B', 'Matrix-1C', 'Matrix-1D', 'Matrix-1E', 
                               'Matrix-2A', 'Matrix-2B', 'Matrix-2C', 'Matrix-3A'], # 使用实际的大写格式
                        multi=True,
                        clearable=True,
                ...
                html.Div([
                    ...
                    dcc.Dropdown(
                        id='severe-classification-dropdown',
                        ...
                        value=['Showstopper_Candidate', 'Showstopper_Confirmed', 'Preventing Maturity Grade ConDrive', 
                               'Obstructing Maturity Grade ConDrive'], # 预设严重的classification
                        multi=True,
                        clearable=True,
        ...
        if 'child_count_of_master' in ddf.columns:
            ...
            # 如果没有child_count_of_master列，则创建一个默认的
        ...
        return html.Div([
            ...
            html.Div([
                ...
                dcc.Graph(
                    ...
                    figure=create_child_complexity_line_chart_hr(ddf, relevant_weeks),
                    style={'height': '450px'}
                )
            ], style={
                'marginBottom': '40px',
            ...
            html.Div([
                ...
                dcc.Graph(
                    ...
                    figure=create_parent_complexity_line_chart_hr(ddf, relevant_weeks),
                    style={'height': '450px'}
                )
            ], style={
                'marginBottom': '40px',
            ...
            html.Div([
                dcc.Graph(id='fv-distribution-chart', style={'display': 'none'}),
                dcc.Graph(id='defect-status-chart', style={'display': 'none'}),
                dcc.Graph(id='solution-cluster-chart', style={'display': 'none'})
            ], style={'display': 'none'})
        ])
        ...
        return html.Div([
            ...
            html.Div([
                html.Div([
                    html.Label('Time Range:', style=active_label_style),
                    dcc.DatePickerRange(
                        id='date-range-picker-main',
                        start_date=df['creation_time'].min(),
                        end_date=df['creation_time'].max(),
                        display_format='YYYY-MM-DD',
                        style={'width': '100%'}
                    ),
                ], style={'width': '48%', 'display': 'inline-block', 'marginRight': '2%'}),
                ...
                html.Div([
                    ...
                    dcc.Dropdown(
                        ...
                        multi=True,
                        clearable=True,
                        placeholder='Select Tester (empty=all)...',
                        style={'width': '100%'}
                    ),
                ], style={'width': '48%', 'display': 'inline-block'}),
            ], style={'marginBottom': '20px', 'marginTop': '20px'}),
        ...
        return html.Div([
            ...
            html.Div([
                html.Div([
                    html.Label('Time Range:', style=active_label_style),
                    dcc.DatePickerRange(
                        id='efficiency-date-range-picker',
                        start_date=df['creation_time'].min(),
                        end_date=df['creation_time'].max(),
                        display_format='YYYY-MM-DD',
                        style={'width': '100%'}
                ...
                html.Div([
                    html.Label('Project:', style=active_label_style),
                    dcc.Dropdown(
                        id='efficiency-project-dropdown',
                        options=[{'label': project, 'value': project} for project in sorted(df['project'].apply(lambda x: str(x) if isinstance(x, dict) else x).dropna().unique()) if project],
                        value=[],
                        multi=True,
                        clearable=True,
                        placeholder='Select Project (empty=all)...',
                        style={'width': '100%'}
                    ),
                ], style={'width': '23%', 'display': 'inline-block', 'marginRight': '2%'}),
                
                html.Div([
                    html.Label('AIDA Domain:', style=active_label_style),
                    dcc.Dropdown(
                        id='efficiency-aida-dropdown',
                        options=[{'label': aida, 'value': aida} for aida in sorted(df['aida_english'].apply(lambda x: str(x) if isinstance(x, dict) else x).dropna().unique()) if aida],
                        value=[],
                        multi=True,
                        clearable=True,
                        placeholder='Select AIDA Domain (empty=all)...',
                        style={'width': '100%'}
                    ),
                ], style={'width': '23%', 'display': 'inline-block', 'marginRight': '2%'}),
                ...
                html.Div([
                    html.Label('Status:', style=active_label_style),
                    dcc.Dropdown(
                        id='efficiency-status-dropdown',
                        options=[{'label': status, 'value': status} for status in sorted(df['status_phase'].apply(lambda x: str(x) if isinstance(x, dict) else x).dropna().unique()) if status],
                        value=[],
                        multi=True,
                        clearable=True,
                        placeholder='Select Status (empty=all)...',
                        style={'width': '100%'}
                    ),
            ...
            html.Div([
                ...
                html.Div([
                    html.H3("严重性分布", style=active_label_style),
            ...
            html.Div([
                html.Div([
                    html.H3("缺陷发现效率", style=active_label_style),
                    dcc.Graph(id='defect-discovery-efficiency-chart'),
                ], style={'width': '48%', 'display': 'inline-block', 'marginRight': '2%'}),
                
                html.Div([
                    html.H3("Matrix准确性分析", style=active_label_style),
                    dcc.Graph(id='matrix-accuracy-chart'),
                ], style={'width': '48%', 'display': 'inline-block'}),
            ], style={'marginBottom': '30px'}),
        ...
        # 测试覆盖率分析页面 - 带性能模式选择
        from config import TEST_COVERAGE_PERFORMANCE_CONFIG
        
        return html.Div([
            # 性能模式选择区域
            html.Div([
                html.H3("测试覆盖率分析", style={'marginBottom': '20px'}),
                html.Div([
                    html.Label("性能模式:", style={'fontWeight': 'bold', 'marginRight': '10px'}),
                    dcc.RadioItems(
                        id='test-coverage-performance-mode',
                        options=[
                            {'label': f"🚀 快速模式 (3K条记录)", 'value': 'fast_mode'},
                            {'label': f"📊 完整模式 (全部记录)", 'value': 'full_mode'}
                        ],
                        value='fast_mode',
                        inline=True,
                ...
                # 性能警告提示
                html.Div(id='test-coverage-performance-warning', 
                        style={'color': 'orange', 'fontSize': '14px', 'marginBottom': '20px'}),
                
                # 数据状态显示
                html.Div(id='test-coverage-data-status', 
            ...
            # 动态内容区域
            html.Div(id='test-coverage-dynamic-content'),
            ...
            html.Div([
                ...
                html.Div([
                    html.Div([
                        ...
                        dcc.Graph(
                            id='test-status-distribution-chart-main',
                            config={'displayModeBar': False}
                        )
                    ], className='col-md-6'),
                    html.Div([
...
def update_fv_distribution_chart(projects, start_date, end_date, aidas, statuses, pus, testers, fvs, ecus, lead_models, fvps, severe_matrices, severe_classifications):
    try:
        # 检查数据是否可用
        if df is None or df.empty:
        ...
        # 使用通用筛选函数
        filtered_data = filter_dataframe(df, projects, start_date, end_date, aidas, statuses, pus, testers, fvs, ecus, lead_models, fvs)
...
def update_defect_status_chart(projects, start_date, end_date, aidas, statuses, pus, testers, fvs, ecus, lead_models, fvps, severe_matrices, severe_classifications):
    # 如果statuses为None，使用默认的状态筛选值（与主下拉菜单保持一致）
    if statuses is None:
        statuses = ['01-New', '02-In Pre-Analysis', '03-In Analysis', '04-In Progress', '05-In Testing', '07-In Pre-Verification', '08-In Verification']
    
    # 使用通用筛选函数
    filtered_data = filter_dataframe(df, projects, start_date, end_date, aidas, statuses, pus, testers, fvs, ecus, lead_models, fvps)
...
def update_solution_cluster_chart(projects, start_date, end_date, aidas, statuses, pus, testers, fvs, ecus, lead_models, fvps, severe_matrices, severe_classifications):
    # 如果statuses为None，使用默认的状态筛选值（与主下拉菜单保持一致）
    if statuses is None:
        statuses = ['01-New', '02-In Pre-Analysis', '03-In Analysis', '04-In Progress', '05-In Testing', '07-In Pre-Verification', '08-In Verification']
...
def update_hr_modal(hr_child_click_data, hr_parent_click_data, current_modal_style, current_tab):
    from dash import callback_context
    ...
    # 初始化Modal样式
    modal_style = current_modal_style if current_modal_style else {
        'display': 'none',
        'position': 'fixed',
        'zIndex': '1000',
        'left': '0',
...
def update_modal(fv_click_data, status_click_data, cluster_click_data, close_clicks, 
                current_modal_style, current_tab, projects, start_date, end_date, 
                aidas, statuses, pus, testers, fvs, ecus, lead_models, fvps, 
...
def update_excel_defect_table(projects, start_date, end_date, aidas, statuses, pus, testers, fvs, ecus, lead_models, fvps, severe_matrices, severe_classifications):
    try:
        # 检查数据是否有效
        if df is None or df.empty:
            return [], []
        ...
        # 然后再应用筛选函数到排序后的数据        
        filtered_data = filter_dataframe(df_sorted, projects, start_date, end_date, aidas, statuses, pus, testers, fvs, ecus, lead_models, fvps)
...
def update_testing_kpi_cards(start_date, end_date, selected_testers):
    try:
        # 检查数据是否有效
        if df is None or df.empty:
        ...
        if selected_testers:
            filtered_data = filtered_data[filtered_data['tester'].isin(selected_testers)]
...
def update_tester_efficiency_chart(start_date, end_date, selected_testers):
    # 筛选数据
    filtered_data = df.copy()
...
def update_defect_discovery_trend_chart(start_date, end_date, selected_testers):
    # 筛选数据
    filtered_data = df.copy()
...
def update_tester_severity_chart(start_date, end_date, selected_testers):
    # 筛选数据
    filtered_data = df.copy()
...
def update_defect_status_distribution_chart(start_date, end_date, selected_testers):
    # 筛选数据
    filtered_data = df.copy()
...
def update_aida_coverage_chart(start_date, end_date, selected_testers):
    # 筛选数据
    filtered_data = df.copy()
...
def update_project_contribution_chart(start_date, end_date, selected_testers):
    # 筛选数据
    filtered_data = df.copy()
...
def update_tester_stats_table(start_date, end_date, selected_testers):
    # 筛选数据
    filtered_data = df.copy()
...
@app.callback(
    ...
    [Input('efficiency-date-range-picker', 'start_date'),
     Input('efficiency-date-range-picker', 'end_date'),
     Input('efficiency-project-dropdown', 'value'),
     Input('efficiency-aida-dropdown', 'value'),
     Input('efficiency-status-dropdown', 'value')]
)
def update_efficiency_kpi_cards(start_date, end_date, selected_projects, selected_aidas, selected_statuses):
    try:
        # 检查数据是否有效
        if df is None or df.empty:
            return html.Div("数据加载中...", style={'textAlign': 'center', 'padding': '20px'})
...
# 缺陷发现效率图表回调
@app.callback(
    Output('defect-discovery-efficiency-chart', 'figure'),
    [Input('efficiency-date-range-picker', 'start_date'),
     Input('efficiency-date-range-picker', 'end_date'),
     Input('efficiency-project-dropdown', 'value'),
     Input('efficiency-aida-dropdown', 'value'),
...
def update_defect_discovery_efficiency_chart(start_date, end_date, selected_projects, selected_aidas, selected_statuses):
    # 筛选数据
    filtered_data = df.copy()
    ...
    # 按日期分组计算缺陷发现效率
    filtered_data['date'] = pd.to_datetime(filtered_data['creation_time']).dt.date
    daily_counts = filtered_data.groupby(['date', 'severity_group']).size().reset_index(name='count')
...
def update_defect_matrix_chart(start_date, end_date, selected_projects, selected_aidas, selected_statuses):
    # 筛选数据
    filtered_data = df.copy()
...
def update_defect_density_chart(start_date, end_date, selected_projects, selected_aidas, selected_statuses):
    # 筛选数据
    filtered_data = df.copy()
...
def update_severity_distribution_chart(start_date, end_date, selected_projects, selected_aidas, selected_statuses):
    # 筛选数据
    filtered_data = df.copy()
    ...
    # 计算严重性分布
    severity_counts = filtered_data['severity_group'].value_counts().reset_index()
    severity_counts.columns = ['severity_group', 'count']
    
    # 创建饼图
    fig = px.pie(
        severity_counts,
        values='count',
        names='severity_group',
        color='severity_group',
        color_discrete_map=SEVERITY_COLORS,
        title="缺陷严重性分布"
    )
    
    fig = apply_chart_style(fig, title="缺陷严重性分布")
...
# Matrix准确性分析图表回调
@app.callback(
    Output('matrix-accuracy-chart', 'figure'),
    [Input('efficiency-date-range-picker', 'start_date'),
     Input('efficiency-date-range-picker', 'end_date'),
     Input('efficiency-project-dropdown', 'value'),
     Input('efficiency-aida-dropdown', 'value'),
...
def update_matrix_accuracy_chart(start_date, end_date, selected_projects, selected_aidas, selected_statuses):
    # 筛选数据
    filtered_data = df.copy()
    ...
    # 统计各Matrix的分布
    matrix_counts = filtered_data['matrix_display'].value_counts().head(15).reset_index()
    matrix_counts.columns = ['matrix_display', 'count']
...
def global_search(search_clicks, search_submit, search_value):
    """全局搜索功能"""
    from dash import callback_context
...
def export_to_excel(n_clicks):
    ...
    try:
        # 使用通用筛选函数
        filtered_data = filter_dataframe(df, [], None, None, [], [], [], [], [], [], [], [])
...
def export_cluster_to_excel(n_clicks):
    """
...
# Test Status Analysis回调函数
@app.callback(
    [Output('test-status-fv-chart-main', 'figure'),
     Output('test-status-distribution-chart-main', 'figure'),
...
def update_test_status_charts(current_nav, projects, test_weeks, fvs, statuses, aidas):
    ...
    try:
        ...
        def map_status_to_display_name(status_value):
            """将原始状态值映射为显示名称"""
            status_mapping = {
                'passed': 'Passed',
                'failed': 'Failed', 
            ...
            return status_mapping.get(str(status_value).lower(), str(status_value))
        ...
        if 'native_status' in filtered_data.columns:
            print("Debug: 应用native_status状态映射")
            filtered_data['native_status'] = filtered_data['native_status'].apply(map_status_to_display_name)
        elif 'status' in filtered_data.columns:
            print("Debug: 应用status状态映射")
            filtered_data['status'] = filtered_data['status'].apply(map_status_to_display_name)
        ...
        if statuses and len(statuses) > 0 and status_col:
            filtered_data = filtered_data[filtered_data[status_col].astype(str).isin([str(s) for s in statuses])]
        ...
        def create_fv_stacked_chart():
            try:
                ...
                fig = px.bar(
                    ...
                    color='status',
                    title="FV vs 测试用例MR数量 (按执行状态分组)",
                    labels={'count': '测试用例MR数量', 'fv': 'FV', 'status': '执行状态'},
                    barmode='stack',
        ...
        def create_status_distribution_chart():
            try:
                ...
                status_counts = filtered_data[status_col].value_counts()
        ...
        return create_fv_stacked_chart(), create_status_distribution_chart(), create_project_count_chart()
