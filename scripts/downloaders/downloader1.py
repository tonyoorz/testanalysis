...
def extract_and_download_related_defects(session, defect_data, output_dir, year_str, save_csv_flag, save_excel_flag, limit_per_page):
    ...
    # Explicitly create the chunks list first
    ...
    for chunk_idx, chunk in enumerate(chunks): # Now iterate over the defined 'chunks' variable
        id_conditions = [f"id='{master_id}'" for master_id in chunk]
        id_query = "||".join(id_conditions)
        query = f'"({id_query})"'
        
        logging.info(f"下载 Master 缺陷数据块 {chunk_idx + 1}/{len(chunks)}...")
...
def main():
    ...
    if not auth_method_selected: # If no auth method provided via command line, ask interactively
        while True:
            print("\n选择认证方法:\n1: SSO (需要 sso_session 和 login_info.txt)\n2: Cookie (使用代码中预定义的)")
